...
def _perfect_paper() -> tuple[float, float]:
    """Điểm knowledge/listening khi trả lời đúng 100%, trên phân bố difficulty giả định.

    Trên đề THẬT thì test_placement_forms lo — file này chỉ kiểm luật chấm, không kiểm đề.
    """
    k_scores = [sc.score_mcq(d, True, 0, False) for d in [1, 3, 5, 1, 3, 4]]
    l_scores = [sc.score_mcq(d, True, 0, True) for d in [1, 2, 3, 4, 5]]
    return (
...
def test_tra_loi_dung_het_va_noi_tot_thi_dat_b1():
    ...
    verdict = sc.decide({'knowledge': knowledge, 'listening': listening, 'speaking': 95.0,
                         'reading': 92.0, 'writing': 90.0},
                        speech_available=True, silent_count=0)
...
def test_khong_bao_gio_tut_qua_mot_bac(knowledge, listening, speaking, speech):
    verdict = sc.decide({'knowledge': knowledge, 'listening': listening, 'speaking': speaking,
                         'reading': listening, 'writing': knowledge}, speech, silent_count=0)
        ...
        for lis in range(0, 101, 5):
            for sp in range(0, 101, 10):
                for speech in (True, False):
                    v = sc.decide({'knowledge': float(k), 'listening': float(lis),
                                   'speaking': float(sp), 'reading': float(lis),
                                   'writing': float(k)}, speech, silent_count=0)
...
def test_gioi_giay_yeu_noi_khong_bi_day_ve_pre_a1():
    ...
    verdict = sc.decide({'knowledge': 90.0, 'listening': 90.0, 'speaking': 29.0,
                         'reading': 90.0, 'writing': 85.0}, speech_available=True, silent_count=0)
...
def test_micro_hong_van_xep_duoc_tu_hai_truc_con_lai():
    verdict = sc.decide({'knowledge': 60.0, 'listening': 75.0, 'reading': 70.0,
                         'writing': 65.0}, speech_available=False, silent_count=0)
    assert verdict.band != Cefr.PRE_A1
...
def test_im_lang_that_thi_pre_a1():
    verdict = sc.decide({'knowledge': 80.0, 'listening': 80.0, 'speaking': 0.0,
                         'reading': 80.0, 'writing': 75.0},
                        speech_available=True, silent_count=4)
    assert verdict.band == Cefr.PRE_A1
    assert verdict.confidence == "low"
...
def test_lech_ky_nang_lon_thi_confidence_thap():
    verdict = sc.decide({'knowledge': 95.0, 'listening': 95.0, 'speaking': 20.0,
                         'reading': 95.0, 'writing': 90.0},
                        speech_available=True, silent_count=0)
    assert verdict.confidence == "low"


def test_deu_ba_truc_va_xa_moc_cat_thi_confidence_cao():
    verdict = sc.decide({'knowledge': 70.0, 'listening': 70.0, 'speaking': 70.0,
                         'reading': 70.0, 'writing': 70.0},
                        speech_available=True, silent_count=0)
    assert verdict.confidence == "high"


def test_tra_loi_cham_bat_thuong_chi_ha_confidence_khong_tru_diem():
    deu_scores = {'knowledge': 70.0, 'listening': 70.0, 'speaking': 70.0,
                  'reading': 70.0, 'writing': 70.0}
    deu = sc.decide(deu_scores, True, 0, slow_ratio=0.0)
    cham = sc.decide(deu_scores, True, 0, slow_ratio=0.9)
...
def test_moc_cat_hieu_dung_dung_nhu_thiet_ke(overall, mong_doi):
    ...
    """
    verdict = sc.decide({k: overall for k in sc.WEIGHTS},
                        speech_available=True, silent_count=0)
