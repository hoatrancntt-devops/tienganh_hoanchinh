...
**Hiện có 70 bài** trải bốn level Pre-A1 → B1. Thang level, tuyên bố đầu ra từng kỹ năng và bảng ánh xạ bài ↔ level nằm ở [`docs/khung-level.md`](docs/khung-level.md).

**Cả 70 bài đều có đủ bốn kỹ năng** — nghe, nói, đọc, viết. Bài đọc là văn bản liền không sinh audio, câu hỏi bằng tiếng Anh; bài viết chấm bằng luật ngay tại máy chủ, không cần API key.
...
make psql                        # mở psql
make test                        # 153 test (SQLite in-memory, không cần Docker)
make dev                         # hot reload
